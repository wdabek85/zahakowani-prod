...
namespace Isolated\Inpost_Pay\Isolated_Guzzlehttp\GuzzleHttp\Handler;

use Isolated\Inpost_Pay\Isolated_Guzzlehttp\GuzzleHttp\Exception\ConnectException;
use Isolated\Inpost_Pay\Isolated_Guzzlehttp\GuzzleHttp\Exception\RequestException;
use Isolated\Inpost_Pay\Isolated_Guzzlehttp\GuzzleHttp\Promise as P;
use Isolated\Inpost_Pay\Isolated_Guzzlehttp\GuzzleHttp\Promise\FulfilledPromise;
use Isolated\Inpost_Pay\Isolated_Guzzlehttp\GuzzleHttp\Promise\PromiseInterface;
use Isolated\Inpost_Pay\Isolated_Guzzlehttp\GuzzleHttp\Psr7\LazyOpenStream;
use Isolated\Inpost_Pay\Isolated_Guzzlehttp\GuzzleHttp\TransferStats;
use Isolated\Inpost_Pay\Isolated_Guzzlehttp\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Isolated\Inpost_Pay\Isolated_Guzzlehttp\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Isolated\Inpost_Pay\Isolated_Guzzlehttp\GuzzleHttp\Psr7\Utils::streamFor($sink);
